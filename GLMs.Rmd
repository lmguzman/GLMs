...
```{r}
OwlModelLM <- lm(n ~ sun + height + species, data = Owl)

OwlModel <- glm(n ~ sun + height + species, family = poisson (link = log), data = Owl)
```
...
```{r}
OwlModel$deviance / OwlModel$df.residual
```
...
```{r}
(OwlModel$null.deviance - OwlModel$deviance) / OwlModel$null.deviance
```
...
Looking at the summary, Tb is a categorical variables with two categories. Therefore we need to use the binomial distribution. 
...
Look at all the other link functions and choose the one with the lowest AIC
